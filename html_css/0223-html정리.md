...
BFC는 웹 페이지를 렌더링하는 시각적 CSS의 일부로서, 블록 박스의 레이아웃이 발생하는 지점과 플로팅 요소의 상호작용 범위를 결정하는 범위이다. 페이지의 width 한줄을 다 차지하고 다음 block의 위치를 계산할때 첫번째 block요소의 아래에 위치하게 된다.

### 👉 가상요소 선택자 ::after, ::before
가상요소 선택자를 이용해 문서의 특정 부분을 스타일링 할 수 있다. 예를 들면 구분선을 넣거나, 앞 뒤에 이미지를 넣거나 할때 사용이 되어진다. ```::after```은 선택된 요소의 뒤에 내용을 생성한다. ```::before```은 선택된 요소의 앞에 내용을 선택한다. 이 선택자들을 사용할때 ```content:""```는 값으로 필수적으로 들어가야 한다.
<br>

아래의 p태그에 ::before 와 ::after를 이용해 원하는 기호를 넣어보았다. p를 기준으로 앞뒤에 content에 넣은 기호가 들어가 있는 것을 확인 할 수 있다.
...
__flex는 요소들을 자유자재로 위치시킬 수 있는 속성이다__ 부모에 display:flex를 주게되면 해당 태그는 부모가 되고 태그 안에 있는 요소들이 자식요소가 된다. 이때 자식요소가 inline의 성격을 가지고 있어도 ```flex item```이 되므로  높이나 길이값을 가질 수 있게 된다. 부모요소는 flex container라고 부르게 되고 자식요소는 flex item이라고 부르게 된다. container가 flex의 영향을 받는 전체 공간이고 설정된 속성에 따라 각각의 item들이 어떠한 형태로 배치가 된다.   
...
![flex](https://user-images.githubusercontent.com/64240637/109385837-fe364b80-7939-11eb-8a98-11b5b4824fbe.png)
...
```text
<address>, <article>, <as👉ide>, <audio>, <blockquote>, <canvas>, <dd>, <div>, <dl>, <fieldset>, <figcaption>, <figure>, <footer>, <form>, <h1>, <h2>, <h3>, <h4>, <h5>, <h6>,<li> <header>, <hgroup>, <hr>, <noscript>, <ol>, <output>, <p>, <pre>, <section>, <table>, <ul>, <video>
```
...
absolute 속성값은 브라우저가 문서의 흐름과 상관없이 요소를 어느쪽에 놓을지 상하좌우 속성값을 이용하여 위치시키는 속성값이다. 이때 기준이 되는 위치는 position:relative가 있는 부모요소나 조상요소이다.
이때 position:relative가 없다면 body를 기준으로 붙게된다
```